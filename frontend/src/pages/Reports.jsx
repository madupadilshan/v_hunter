...
function Reports({ theme = 'dark' }) {
  ...
  return (
    <div
      className={`w-full min-h-screen relative overflow-x-hidden pt-32 pb-10 ${
        isDark ? 'bg-[#050511]' : 'bg-gradient-to-br from-slate-50 via-emerald-50 to-cyan-50'
      }`}
    >
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold mb-2">
              <span className={isDark ? 'text-gray-100' : 'text-slate-900'}>REPORTS</span>
              <span className="text-green-500 drop-shadow-lg"> ARCHIVE</span>
            </h1>
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>Access and manage your security analysis reports</p>
          </div>
          <div className="bg-green-500/10 border border-green-500/50 rounded-lg px-6 py-3">
            ...
            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>Total Scans</p>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {reports.length === 0 ? (
                  <tr>
                    <td colSpan={6} className={`px-6 py-12 text-center text-sm ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>
                      {emptyStateMessage}
                    </td>
                  </tr>
                ) : (
                  reports.map((report) => (
                    <tr
                      ...
                      className={`border-b transition-colors ${
                        isDark
                          ? 'border-gray-700/30 hover:bg-gray-900/20'
                          : 'border-slate-300 hover:bg-slate-100/70'
                      }`}
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <FileText className="w-5 h-5 text-cyan-400" />
                          <p className={`font-semibold ${isDark ? 'text-gray-100' : 'text-slate-900'}`}>{report.name}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className={`flex items-center gap-2 text-sm ${isDark ? 'text-gray-300' : 'text-slate-700'}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          <button
                            className={`p-2 rounded-lg transition-colors text-cyan-400 hover:text-cyan-300 ${
                              isDark ? 'hover:bg-gray-900/50' : 'hover:bg-slate-200'
                            }`}
                          >
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex justify-between items-center">
                  <span className={isDark ? 'text-gray-300' : 'text-slate-700'}>{item.severity}</span>
